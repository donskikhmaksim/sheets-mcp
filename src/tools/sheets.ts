...
export function registerSheetsTools(server: McpServer, clients: UserClients, ctx: SheetsConsentContext = DEFAULT_CONSENT_CTX) {
  ...
  server.registerTool(
    ...
    guard(async ({ account, spreadsheets, manifest_id, user_reply }) => {
      ...
      const { payload, auditId } = decision;
      // Bounded fan-out (mapWithLimit, cap 8) instead of an unbounded
      // Promise.all — a large batch of spreadsheets used to fire every
      // .create() call at once, tripping Sheets/Drive per-user rate limits.
      // The 429/5xx retry lives in `withRetry`, wrapped around just the one
      // network call so a per-item try/catch below still captures a final
      // failure into `results` instead of throwing.
      ...
      return buildMutationResult({
        ...
        // verbatim (it's the create input, echoed back) — re-fetching would
        // just duplicate it under a second key.
      });
    }),
  );
