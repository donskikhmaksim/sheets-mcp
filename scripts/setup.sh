...
if [[ -n "$GH_USER" ]]; then
  ...
  if [[ -z "$GH_USER" ]]; then
    echo -e "${YELLOW}⚠️  Продолжаю без форков — Railway подключится напрямую к апстриму.${RESET}"
    echo -e "${YELLOW}   Обновления придётся подтягивать вручную (переустановкой).${RESET}"
  fi
fi
